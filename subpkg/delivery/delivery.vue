<template>
  ...
  <view class="order-container">
      ...
      <view class="navbar">
        ...
        <view class="order-icon" onclick="toggleOrder(this)">
          <view class="order-box">
            <image src="https://ysc-1328676482.cos.ap-nanjing.myqcloud.com/my-icons/booking.png" alt="订单"
              class="order-img" />
    ...
    <view class="search-container">
      ...
      <!-- 下拉菜单 -->
      <view v-if="showDropdown" class="dropdown-menu" @mousedown.stop.prevent="preventBlur">
        <view class="dropdown-item" v-for="item in recentSearches" :key="item" @click="selectSearch(item)">
          {{ item }}
        </view>
      </view>
    </view>
    ...
    <view class="section">
      ...
      <view class="icon-container">
        ...
        <view class="icon">
          <image src="https://ysc-1328676482.cos.ap-nanjing.myqcloud.com/my-icons/weizhi.png" class="icon-image" />
          <text class="icon-text">服装编号</text>
        </view>
    ...
    <view class="section">
      ...
      <view class="form-item">
        ...
        <!-- Picker 组件用于选择衣柜位置 -->
        <picker mode="selector" :range="cloudCabinetList" range-key="name" @change="onLockerLocationChange">
          <view class="picker">
            <!-- 显示选择的衣柜位置 -->
      ...
      <view class="form-item">
        <text>服装编号：</text>
        <input type="number" placeholder="请输入服装编号" v-model="formData.lockerNumber" />
      </view>
    </view>
    ...
    <view class="section">
      ...
      <view class="form-item">
        <text>手机号：</text>
        <input type="number" maxlength="11" placeholder="请输入手机号" v-model="phoneNumber" class="input-field" />
      </view>

      <!-- 验证码输入和发送按钮 -->
      <view class="form-item">
        <text>验证码：</text>
        <input type="number" maxlength="6" placeholder="请输入验证码" v-model="verificationCode"
          class="input-field code-input" />
        <button class="send-code-btn-inline" :disabled="isButtonDisabled" @click="sendVerificationCode">
  ...
  export default {
    data() {
      return {
        ...
        pickUpTime: '', // 取件时间
        formData: {
          lockerLocation: '', // 可以保留原来的字段
          lockerId: null, // 建议增加id字段
          lockerNumber: ''
        },
        // 衣柜位置选项
        lockerOptions: ['河南省郑州市中原区长椿路141号', '河南省郑州市中原区长椿路辅路', '河南省郑州市中原区科学大道', '河南省郑州市中原区翠竹街辅路', '河南省郑州市中原区须水河东路',
          '河南省郑州市中原区梧桐街', '河南省郑州市中原区西四环', '河南省郑州市中原区白桦街', '河南省洛阳市老城区马市街121号'
        ],
        cloudCabinetList: [],
        selectedCabinetId: null // 用于存储选中的id
      };
    },
    computed: {
      // 按钮是否禁用
    ...
    methods: {
      async getTerminalList() {
        ...
        this.cloudCabinetList = res.data
        console.log(this.cloudCabinetList)
      },
      ...
      async submitOrder() {
        ...
        const params = {
          siteId: this.formData.lockerId, // 注意参数名与后端一致
          hanFuId: this.formData.lockerNumber // 注意大小写（建议统一）
        };
        ...
        const {
          data: res
            ...
            `/lease/appointmentOrder?${queryString}`)
          ...
          onLockerLocationChange(e) {
            ...
            const selectedItem = this.cloudCabinetList[index]
            this.selectedCabinetName = selectedItem.name
            this.formData.lockerLocation = selectedItem.name
            this.formData.lockerId = selectedItem.cabinetId // 同时保存id
          },
...
<style>
  ...
  .form-item input:focus {
    border-color: #9d0f0f;
    /* 更新颜色 */
    box-shadow: 0 0 4px rgba(157, 15, 15, 0.3);
    /* 更新颜色 */
  }
  ...
  .fullscreen-gradient-bg {
    ...
    z-index: -1;
    /* 优先级最低 */
    background: linear-gradient(to bottom, #a10f0f, #ffffff);
    /* 渐变背景 */
  }
